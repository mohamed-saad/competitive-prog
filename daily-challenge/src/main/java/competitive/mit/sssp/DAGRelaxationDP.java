...
public class DAGRelaxationDP implements ShortestPath {
	...
	private int shortestPath(Character from, Character to, Map<Character, List<Edge>> adj) {
		String key = from + "-" + to;
		...
		int min = INFINIT;
		for (Edge e: n) {
			int path = shortestPath(from, e.c, adj) + e.weight;
			if (path < min)
				min = path;
		}
		memo.put(key, min);
	...
	public Map<Character, Integer> shortestPath(Character from, List<String> edges) {
		...
		for (Character n: nodes) {
			shortestPath.put(n, shortestPath(from, n, adj));
		}
